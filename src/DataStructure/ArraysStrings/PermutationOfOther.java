...
public class PermutationOfOther {
	...
	{/*
		...
		{
			if(hashMap.containsKey(str11[i]))
			{
				//
				hashMap.put(hashMap.get(str11[i])+1, str11[i]);
			}
			else
			{
				hashMap.put(1,str11[i]);
			}
		}
		
		for(int i=0;i<str22.length;i++)
		{
			if(hashMap.containsKey(str22[i]))
			{
				hashMap.put(hashMap.get(str22[i])-1,str22[i]);
